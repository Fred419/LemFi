...
// Populate flags in country menu
document.querySelectorAll("[data-flag]").forEach((el) => {
  el.innerHTML = FLAGS[el.dataset.flag] || "";
});


// Open or close country menu
...
countryToggle.addEventListener("click", (e) => {
  if (e.target.closest(".country-menu")) return;

  openMenu(!countryMenu.classList.contains("open"));
});


// Keyboard controls
countryToggle.addEventListener("keydown", (e) => {
...
document.addEventListener("click", (e) => {
  if (!countryToggle.contains(e.target)) {
    openMenu(false);
  }
});
...
appNotice.addEventListener("click", (e) => {
  if (e.target === appNotice) {
    hideAppNotice();
  }
});
